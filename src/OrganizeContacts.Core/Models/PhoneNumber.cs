...
public sealed class PhoneNumber
{
    ...
    public static PhoneNumber Parse(string raw, PhoneKind kind = PhoneKind.Other, bool preferred = false)
    {
        var digits = new string(raw.Where(char.IsDigit).ToArray());
        return new PhoneNumber { Raw = raw, Digits = digits, Kind = kind, IsPreferred = preferred };
    }
}
